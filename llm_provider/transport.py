...
class ResponsesAPICompletion:
    ...
    def __call__(
        ...
        *,
        ...
        role = schema.get("title", "unknown")
        try:
            response = self._responses_create(system, user, schema, max_tokens)
        except Exception as exc:
            raise LlmProviderError(f"responses-api request failed: {exc}") from exc

        status = getattr(response, "status", None)
        incomplete = getattr(response, "incomplete_details", None)
        stop_reason = getattr(incomplete, "reason", None) or getattr(response, "finish_reason", None) or status
        latency_ms = int((time.monotonic() - started) * 1000)
        usage = getattr(response, "usage", None)
        self.usage.append(
            UsageRecord(
        ...
        if status in {"incomplete", "failed"}:
            raise LlmProviderError(f"responses-api stopped with {stop_reason}")

        text = _responses_text(response)
        try:
            parsed = json.loads(text)
        except json.JSONDecodeError as exc:
            raise LlmProviderError(f"responses-api returned invalid JSON: {exc}") from exc
        if not isinstance(parsed, dict):
            raise LlmProviderError("responses-api JSON output must be an object")
        return parsed

    def _responses_create(self, system: str, user: str, schema: dict, max_tokens: int):
        client = self._get_client()
        request_user = user
        if self.json_mode == "schema":
            text = {
    ...
    for item in getattr(response, "output", []) or []:
        for content in getattr(item, "content", []) or []:
            ...
            if isinstance(text, str):
                return text
    raise LlmProviderError("responses-api response did not contain text output")
